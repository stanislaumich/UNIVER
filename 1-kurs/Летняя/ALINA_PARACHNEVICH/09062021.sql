...
-- Дамп данных таблицы pas-trans.firma: ~0 rows (приблизительно)
DELETE FROM `firma`;
...
-- Дамп данных таблицы pas-trans.transport: ~0 rows (приблизительно)
...
-- Дамп данных таблицы pas-trans.zakaz: ~0 rows (приблизительно)
DELETE FROM `zakaz`;
...
-- Дамп структуры для представление pas-trans.zd_a
DROP VIEW IF EXISTS `zd_a`;
-- Создание временной таблицы для обработки ошибок зависимостей представлений
CREATE TABLE `zd_a` (
	`naimen` VARCHAR(500) NOT NULL COLLATE 'utf8mb4_0900_ai_ci',
	`adress` VARCHAR(5000) NOT NULL COLLATE 'utf8mb4_0900_ai_ci',
	`cena` INT(10) UNSIGNED NOT NULL,
	`kolvo` INT(10) UNSIGNED NOT NULL
) ENGINE=MyISAM;

-- Дамп структуры для представление pas-trans.zd_b
DROP VIEW IF EXISTS `zd_b`;
-- Создание временной таблицы для обработки ошибок зависимостей представлений
CREATE TABLE `zd_b` (
	`fio` VARCHAR(500) NOT NULL COLLATE 'utf8mb4_0900_ai_ci'
) ENGINE=MyISAM;

-- Дамп структуры для представление pas-trans.zd_c
DROP VIEW IF EXISTS `zd_c`;
-- Создание временной таблицы для обработки ошибок зависимостей представлений
CREATE TABLE `zd_c` (
	`naimen` VARCHAR(500) NOT NULL COLLATE 'utf8mb4_0900_ai_ci',
	`kolvo` INT(10) UNSIGNED NOT NULL
...
-- Дамп структуры для представление pas-trans.zd_d
DROP VIEW IF EXISTS `zd_d`;
-- Создание временной таблицы для обработки ошибок зависимостей представлений
CREATE TABLE `zd_d` (
	`fio` VARCHAR(250) NOT NULL COLLATE 'utf8mb4_0900_ai_ci',
	`dolgznost` VARCHAR(500) NOT NULL COLLATE 'utf8mb4_0900_ai_ci'
) ENGINE=MyISAM;

-- Дамп структуры для представление pas-trans.zd_e
DROP VIEW IF EXISTS `zd_e`;
-- Создание временной таблицы для обработки ошибок зависимостей представлений
CREATE TABLE `zd_e` (
	`fio` VARCHAR(500) NOT NULL COLLATE 'utf8mb4_0900_ai_ci',
...
DROP TABLE IF EXISTS `zd_a`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `zd_a` AS select `t`.`naimen` AS `naimen`,`s`.`adress` AS `adress`,`t`.`cena` AS `cena`,`t`.`Kolvo` AS `kolvo` from (`tovar` `t` join `sklad` `s`) where ((`t`.`idsklad` = `s`.`Nsklad`) and (`t`.`naimen` = 'Ноутбук') and (`s`.`Nsklad` = 1));

-- Дамп структуры для представление pas-trans.zd_b
DROP VIEW IF EXISTS `zd_b`;
-- Удаление временной таблицы и создание окончательной структуры представления
DROP TABLE IF EXISTS `zd_b`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `zd_b` AS select `p`.`fio` AS `fio` from (`postav` `p` join `tovar` `t`) where ((`t`.`idpostav` = `p`.`idpostav`) and (`t`.`naimen` = 'Мышка') and (`t`.`cena` <> 10));

-- Дамп структуры для представление pas-trans.zd_c
DROP VIEW IF EXISTS `zd_c`;
-- Удаление временной таблицы и создание окончательной структуры представления
DROP TABLE IF EXISTS `zd_c`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `zd_c` AS select `t`.`naimen` AS `naimen`,`n`.`kolvo` AS `kolvo` from (`nakladnaya` `n` join `tovar` `t`) where ((`n`.`idtovar` = `t`.`idtovar`) and (`n`.`idnakladnaya` = 2) and (`n`.`summa` = 100));

-- Дамп структуры для представление pas-trans.zd_d
DROP VIEW IF EXISTS `zd_d`;
-- Удаление временной таблицы и создание окончательной структуры представления
DROP TABLE IF EXISTS `zd_d`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `zd_d` AS select `k`.`fio` AS `fio`,`k`.`dolgznost` AS `dolgznost` from (`kadry` `k` join `postav` `p`) where (`k`.`fio` = `p`.`fio`);

-- Дамп структуры для представление pas-trans.zd_e
DROP VIEW IF EXISTS `zd_e`;
-- Удаление временной таблицы и создание окончательной структуры представления
DROP TABLE IF EXISTS `zd_e`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `zd_e` AS select `p`.`fio` AS `fio`,`s`.`bank` AS `bank`,`s`.`numchet` AS `numchet` from (`schet` `s` join `postav` `p`) where ((`p`.`fio` = 'ЧТУП АксаЛайт') and (`p`.`idschet` = `s`.`id`));
